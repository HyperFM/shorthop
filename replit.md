...
- **Ride Matching & Preferences**: Simplified binary matching system — match or no match. Conditions: time window ≤30min, pickup within 0.5mi of driver, same-direction travel (bearing check ≤90°), destination alignment with driver's saved routes, seat availability (seatsNeeded ≤ availableSeats), no-stranding dropoff rule. No ranking/scoring. Driver Matching Style selector with three options stored as `allowDetourDrivers` text field (values: "both", "non_detour_only", "detour_only").
