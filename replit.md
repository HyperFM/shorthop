...
- **MagicGPS Detection**: Smart Route Memory system with movement detection and auto-hop activation. `magicGpsEnabled` boolean on users table. `saved_routes` table (id, userId, name, address, lat, lng, confirmCount). CRUD routes: GET/POST/PUT/DELETE `/api/saved-routes`, POST `/api/saved-routes/:id/confirm`. Frontend hook `use-magic-gps.ts` tracks GPS position, calculates speed (walking <7mph, driving >7mph), bearing, and matches movement direction against saved routes using 45° bearing cone with confirmCount boost. 10-min notification cooldown, 15-min post-decline cooldown. MagicGpsSection component in Dashboard (both driver+hopper tabs) with toggle, saved routes list (add/edit/delete), and Mapbox geocoding on save. MagicGpsSuggestion/MagicGpsActivation/MagicGpsStatus notification overlays in InstaHop. Browser Notification API integration.
- **Intelligent Commute Layer (FlexHop/PowerHop)**: Built on MagicGPS with 9 features:
  - **Flow Mode**: `flowModeEnabled` on users. Auto-activates silently when confidence threshold met (confirmCount ≥ 3 + bearing confidence ≥ 0.7). Dashboard toggle in MagicGPS section. FlowModeNotification overlay in InstaHop. Activity windows tracked automatically on activation.
  - **Route Sync Matching**: Enhanced `scoreHop` with vector-based direction matching (bearing angle comparison). Drivers heading same direction get priority boost. Corridor alignment + destination matching.
  - **Commute Circles**: `commute_circles` + `commute_circle_members` tables. Create/join/leave/delete circles. Circle members get -8 priority boost in hop scoring. CommuteCircles component in Dashboard driver tab. API: `/api/commute-circles` CRUD + join/leave/members.
  - **Soft Time Windows**: `user_activity_windows` table tracks user activity patterns (day, hour, count). Auto-recorded on hop completion and Flow Mode activation. Used for match prediction enhancement.
  - **On-the-Way Ping**: GET `/api/on-the-way` checks for nearby drivers (< 1.5 miles). InstaHop polls every 30s for hoppers. OnTheWayPing notification overlay: "Someone's heading your way."
  - **Drift Catch**: Walking detection nudge after 2 minutes with 15-min cooldown. DriftCatchNotification overlay: "You're on the move — want a quick hop?"
  - **Micro-Hop**: `microHop` boolean on short_hops. Auto-set for ≤ 1 mile hops. Gets -1 priority boost in scoring for instant match priority.
  - **Confidence System**: `confidenceScore` on users. Routes with high confirm counts + strong bearing matches build confidence for Flow Mode auto-activation.
  - **One-Tap Repeat Routes**: `lastCompletedRouteId`/`lastCompletedRouteName` on users. RepeatRoutePrompt overlay in InstaHop: "Go to X again?" with one-tap activation. Updated on hop completion for both driver and walker.
