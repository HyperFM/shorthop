...
- **Left Behind Safety System**: After a ride starts, if GPS shows driver and hopper are >0.5 miles apart for 90+ seconds (with good GPS accuracy), a "Were you left behind?" prompt appears to the hopper. If confirmed, the ride is auto-cancelled, payment is refunded via Stripe, and the driver receives a strike (`leftBehindStrikes`). 3 strikes = driver account deactivated. GPS distance is double-checked server-side before processing to avoid false positives.
...
- **Community & Social**: A "Network" page for posts, a Friends System with mutual follow requests, and configurable profile privacy. Supports emoji reactions and direct messaging. City Chat uses GPS-based Mapbox reverse geocoding to auto-detect the user's city and show a city-specific chat room (e.g., "Lexington ShortHop"). City chat data is stored in `city_messages` table keyed by city name. Requires FlexHop+ subscription. Founders Chat remains completely separate using `founder_messages` table. Both city chats and Founders chat have a server-side content filter (`server/contentFilter.ts`) that blocks profanity (via `leo-profanity`), ride requests, phone numbers, emails, social media handles, and spam before messages are saved. Blocked messages return 422 with a clear reason shown only to the sender.
